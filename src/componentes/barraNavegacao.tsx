...
export default function BarraNavegacao({ tema, seletorView }: Props) {
                ...
                <div className="absolute left-0 top-full mt-1 w-48 bg-white rounded-md shadow-lg hidden group-hover:block z-50">
                    {grupo.submenu.map((item, subIndex) => (
                        <a
                            ...
                            onClick={(e) => seletorView(`${grupo.nome}-${item}`, e)}
                        >
                            {item}
                        </a>
                    ))}
    ...
    return (
        <nav className="bg-black shadow-md mb-4">
            ...
            {/* Menu Mobile */}
            <div className={`md:hidden ${menuAberto ? 'block' : 'hidden'}`} id="mobile-menu">
                <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gray-900">
                    {gerarListaBotoes().map((item, index) => (
