...
export default function Relatorios({ tema }: Props) {
    const [selectedReport, setSelectedReport] = useState("top10-clientes-qtd");
    
    // Dados fictícios para Top 10 Clientes (Qtd)
    const topClientesQtd: Array<{ cliente: Cliente; quantidade: number }> = [
    ...
    const topProdutosServicos: Array<{ item: Produto | Servico; quantidade: number; valor: number }> = [
    ...
    const consumoPorPet: Array<{ pet: Pet; quantidade: number; valor: number }> = [
    ...
    const topClientesValor: Array<{ cliente: Cliente; valor: number }> = [
    ...
    const renderTopProdutosServicos = () => {
        return (
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
        ...
        return (
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {topClientesValor.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.cliente.nome}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">R$ {item.valor.toFixed(2)}</td>
                            </tr>
    ...
    return (
        <div className="w-full px-4">
            ...
            <div className="mt-8">
                {selectedReport === "top10-clientes-qtd" && renderTopClientesQtd()}
                {selectedReport === "top-produtos-servicos" && renderTopProdutosServicos()}
                {selectedReport === "consumo-por-pet" && renderConsumoPorPet()}
                {selectedReport === "top5-clientes-valor" && renderTopClientesValor()}
            </div>
        </div>
    );
} 